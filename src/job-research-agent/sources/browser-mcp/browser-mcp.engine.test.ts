...
function buildOkTransport(): BrowserMcpTransport {
  return {
    invoke: vi.fn(async () => ({ url: "about:blank", title: "", domHtml: "" })) as unknown as BrowserMcpTransport["invoke"],
    close: vi.fn(async () => {}),
  };
}
...
function buildRawJob(
  sourceId: string,
  overrides: Partial<RawJobPosting> = {},
): RawJobPosting {
  return {
    source: "browser_mcp",
    sourceId,
...
describe("BrowserMcpEngine — adapter throw isolation", () => {
  it("continues past a throwing adapter and aggregates jobs from healthy ones", async () => {
    ...
    const config = buildConfigWithSitesEnabled([
      "linkedin",
      "programathor",
      "glassdoor",
    ]);
    ...
    expect(bySite.get("browser_mcp:programathor")?.error).toBe(
      "programathor blew up",
    );
