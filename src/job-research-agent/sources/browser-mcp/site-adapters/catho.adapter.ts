...
export const JOB_URL_REGEX = /\/vagas\/(\d+)\//i;
...
function buildCanonicalUrl(href: string): string {
  ...
  return `https://www.catho.com.br${href}`;
}

function buildSearchUrl(query: string, page: number): string {
  const encoded = encodeURIComponent(query);
  let url = `https://www.catho.com.br/vagas/${encoded.replaceAll("%20", "-")}/`;
  if (page > 1) {
    url += `?pagina=${page}`;
  }
  return url;
...
export class CathoAdapter implements SiteAdapter {
  readonly id = "catho" as const;
  ...
  async collect(ctx: SiteAdapterContext): Promise<SiteAdapterResult> {
    ...
    outer: for (const query of siteConfig.searchQueries) {
      ...
      for (let page = 1; page <= maxPages; page += 1) {
        ...
        try {
          await client.navigate("catho", url, siteConfig.rateLimitMs);
          pagesVisited += 1;
