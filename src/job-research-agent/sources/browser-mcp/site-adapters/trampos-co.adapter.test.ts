...
function loadFixture(name: string): string {
  const filePath = path.resolve(process.cwd(), 'test-fixtures', 'browser-mcp', 'trampos-co', name);
...
describe('TramposCoAdapter marketHint', () => {
  ...
  it('não define marketHint quando locationText não contém termos brasileiros', async () => {
    const htmlWithForeignLocation = `
      <main>
        <div class="opportunity-box" data-opportunity-id="55555">
          <h4>Frontend Dev</h4>
          <img alt="ForeignCo">
          <span class="location">New York, USA</span>
          <a href="/oportunidades/55555-frontend-dev">Ver vaga</a>
        </div>
      </main>
    `;
...
describe('Property 2 — URL round-trip', () => {
  ...
  function buildTramposHtml(jobId: string, slug: string): string {
    return `
      <main>
        <div class="opportunity-box" data-opportunity-id="${jobId}">
          <h4>Test Engineer</h4>
          <img alt="Test Corp">
          <span class="location">São Paulo, SP</span>
          <a href="/oportunidades/${jobId}-${slug}">Ver vaga</a>
        </div>
      </main>
    `;
  }
