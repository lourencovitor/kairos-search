...
// aria-label="Ir para vaga TITLE da empresa COMPANY na cidade CITY publicada há X dias..."
const ARIA_REGEX = /aria-label="Ir para vaga (.+?) da empresa (.+?) na cidade (.+?) publicada/i;

function extractTitle(cardHtml: string): string | undefined {
  const match = ARIA_REGEX.exec(cardHtml);
  return match ? normalizeWhitespace(match[1]) : undefined;
}

function extractCompany(cardHtml: string): string | undefined {
  const match = ARIA_REGEX.exec(cardHtml);
  return match ? normalizeWhitespace(match[2]) : undefined;
}

function extractLocation(cardHtml: string): string | undefined {
  const match = ARIA_REGEX.exec(cardHtml);
  return match ? normalizeWhitespace(match[3]) : undefined;
}
...
function extractJobUuid(cardHtml: string): string | undefined {
...
function buildSearchUrl(query: string, page: number): string {
  ...
  let url = `https://portal.gupy.io/job-search/term=${encoded}`;
  if (page > 1) {
    url += `&page=${page}`;
  }
  return url;
    ...
    outer: for (const query of siteConfig.searchQueries) {
      ...
      for (let page = 1; page <= maxPages; page += 1) {
        ...
        for (const cardHtml of cards) {
          ...
          if (!title || !company || !jobUuid || !href) {
            droppedJobs += 1;
            continue;
          }

          const sourceId = `gupy_public:${jobUuid}`;
