...
import { APPLY_BUTTON_SELECTORS, ReveloAdapter } from "./revelo.adapter.js";
...
function buildMockClient(fixtureHtml: string): BrowserMcpClientLike & {
  ...
  return {
    ...
    snapshot: vi.fn(async () => ({
      url: "https://revelo.com.br/jobs?search=desenvolvedor",
      title: "Vagas de Desenvolvedor | Revelo",
      domHtml: fixtureHtml,
    })),
    click: vi.fn(async () => {}),
    type: vi.fn(async () => {}),
...
describe("ReveloAdapter happy-path fixture", () => {
  it("extrai 3 vagas com shape canônico", async () => {
    ...
    for (const job of result.jobs) {
      ...
      expect(job.sourceBoard).toBe("revelo-browser-mcp");
      ...
      expect(job.sourceId).toMatch(/^revelo:\d+$/);
      expect(job.url).toContain("revelo.com.br");
      ...
      expect(job.metadata).toEqual({
        browserMcp: { siteId: "revelo", query: "desenvolvedor" },
      });
    }

    const ids = result.jobs.map((j) => j.sourceId);
    expect(ids).toEqual(["revelo:111222", "revelo:333444", "revelo:555666"]);
  });
...
describe("ReveloAdapter login-wall fixture", () => {
...
describe("ReveloAdapter captcha fixture", () => {
...
describe("ReveloAdapter pagination", () => {
  it("navega para &page=N em páginas subsequentes", async () => {
    ...
    expect(firstUrl).toContain("revelo.com.br/jobs?search=");
...
describe("ReveloAdapter marketHint", () => {
  ...
  it("não define marketHint quando locationText não contém termos brasileiros", async () => {
    const htmlWithForeignLocation = `
      <div class="job-list">
        <article class="job-card" data-job-id="77777">
          <a class="job-card__title-link" href="https://revelo.com.br/jobs/77777">
            <h2 class="job-card__title">Frontend Dev</h2>
          </a>
...
describe("ReveloAdapter sourceId deduplication", () => {
...
describe("Property 2 — URL round-trip", () => {
  ...
  function buildReveloHtml(jobId: string): string {
    return `
      <div class="job-list">
        <article class="job-card" data-job-id="${jobId}">
          <a class="job-card__title-link" href="https://revelo.com.br/jobs/${jobId}">
            <h2 class="job-card__title">Test Engineer</h2>
          </a>
  ...
  it("URLs emitidas são válidas, absolutas e normalizeJobUrl é idempotente", { timeout: 30_000 }, async () => {
    await fc.assert(
      fc.asyncProperty(jobIdArb, async (jobId) => {
        const html = buildReveloHtml(jobId);
        const client: BrowserMcpClientLike = {
          navigate: vi.fn(async () => {}),
          snapshot: vi.fn(async () => ({
            url: "https://revelo.com.br/jobs",
            title: "Vagas | Revelo",
            domHtml: html,
          })),
          click: vi.fn(async () => {}),
