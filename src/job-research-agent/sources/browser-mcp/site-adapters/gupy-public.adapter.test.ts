...
function loadFixture(name: string): string {
  const filePath = path.resolve(process.cwd(), 'test-fixtures', 'browser-mcp', 'gupy-public', name);
...
describe('GupyPublicAdapter happy-path fixture', () => {
  it('extrai 3 vagas com shape canônico', async () => {
    ...
    for (const job of result.jobs) {
      ...
      expect(job.sourceId).toMatch(
        /^gupy_public:[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/,
      );
...
describe('GupyPublicAdapter marketHint', () => {
  ...
  it('não define marketHint quando locationText não contém termos brasileiros', async () => {
    const htmlWithForeignLocation = `
      <a href="https://foreign.jobs.gupy.io/job/d4e5f6a7-b8c9-0123-def0-456789012345"
         aria-label="Ir para vaga Frontend Dev da empresa ForeignCo na cidade New York publicada há 1 dia">
      </a>
    `;
...
describe('Property 2 — URL round-trip', () => {
  ...
  function buildGupyHtml(uuid: string, companySlug: string): string {
    return `
      <a href="https://${companySlug}.jobs.gupy.io/job/${uuid}"
         aria-label="Ir para vaga Test Engineer da empresa Test Corp na cidade São Paulo publicada há 1 dia">
      </a>
    `;
  }
