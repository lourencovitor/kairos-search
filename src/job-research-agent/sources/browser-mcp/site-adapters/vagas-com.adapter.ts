...
export const CARD_SELECTOR = "a.link-detalhes-vaga";
export const TITLE_SELECTOR = "h2.cargo";
export const COMPANY_SELECTOR = "span.emprNome";
export const LOCATION_SELECTOR = "span.vaga-local";
export const JOB_URL_REGEX = /\/v(\d+)-/i;
...
function splitIntoCards(domHtml: string): string[] {
  // Real: <a class="link-detalhes-vaga" href="/vagas/vID/slug">Title</a> followed by company/location spans
  // We grab a chunk around each link-detalhes-vaga
  const cardRegex =
    /<a\b[^>]*class="link-detalhes-vaga"[^>]*>[\s\S]*?<\/a>[\s\S]*?(?=<a\b[^>]*class="link-detalhes-vaga"|<\/section|$)/gi;
  const out: string[] = [];
...
function extractTitle(cardHtml: string): string | undefined {
  // Real: <a class="link-detalhes-vaga" title="Title">...</a>
  const titleAttr = /title="([^"]+)"/.exec(cardHtml);
  if (titleAttr) return normalizeWhitespace(titleAttr[1]);
  // Fallback: text inside the <a>
  const linkText = /<a[^>]*class="link-detalhes-vaga"[^>]*>([\s\S]*?)<\/a>/i.exec(cardHtml);
  if (linkText) return normalizeWhitespace(linkText[1].replaceAll(/<[^>]+>/g, ""));
  return undefined;
}

function extractCompany(cardHtml: string): string | undefined {
  // Real: <span class="emprVaga">Company</span>
  return extractTextByTag(cardHtml, /<span\b[^>]*class="[^"]*emprVaga[^"]*"[^>]*>([\s\S]*?)<\/span>/i);
}

function extractLocation(cardHtml: string): string | undefined {
  return extractTextByTag(cardHtml, /<span\b[^>]*class="[^"]*vaga-local[^"]*"[^>]*>([\s\S]*?)<\/span>/i);
}

function extractJobUrl(cardHtml: string): string | undefined {
  const hrefMatch = /href="(\/vagas\/v\d+[^"]*)"/.exec(cardHtml);
  if (!hrefMatch) return undefined;
  return hrefMatch[1];
}

function extractJobId(cardHtml: string): string | undefined {
  // data-id-vaga="2813357" or from URL /vagas/v2813357/
  const dataId = /data-id-vaga="(\d+)"/.exec(cardHtml);
...
function buildSearchUrl(query: string, page: number): string {
  ...
  let url = `https://www.vagas.com.br/vagas-de-${encoded.replaceAll("%20", "-")}`;
  if (page > 1) {
    url += `?pagina=${page}`;
  }
  return url;
...
export class VagasComAdapter implements SiteAdapter {
  readonly id = "vagas_com" as const;
  ...
  async collect(ctx: SiteAdapterContext): Promise<SiteAdapterResult> {
    ...
    outer: for (const query of siteConfig.searchQueries) {
      ...
      for (let page = 1; page <= maxPages; page += 1) {
        ...
        try {
          await client.navigate("vagas_com", url, siteConfig.rateLimitMs);
          pagesVisited += 1;
        ...
        for (const cardHtml of cards) {
          ...
          const sourceId = `vagas_com:${jobId}`;
          ...
          const job: RawJobPosting = {
            source: "browser_mcp",
            sourceId,
            sourceBoard: "vagas-com-browser-mcp",
            sourceType: "aggregator",
            sourceFocus: "brazil",
            sourceTier: "brazil_public_api",
            sourceQualityRank: 16,
            companyName: company || "Confidencial",
            title,
