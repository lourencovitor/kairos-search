...
import { APPLY_BUTTON_SELECTORS, CathoAdapter } from "./catho.adapter.js";
...
function buildMockClient(fixtureHtml: string): BrowserMcpClientLike & {
  ...
  return {
    ...
    snapshot: vi.fn(async () => ({
      ...
      title: "Vagas de Desenvolvedor | Catho",
      domHtml: fixtureHtml,
    })),
    click: vi.fn(async () => {}),
    type: vi.fn(async () => {}),
...
describe("CathoAdapter happy-path fixture", () => {
  it("extrai 3 vagas com shape canônico", async () => {
    ...
    for (const job of result.jobs) {
      ...
      expect(job.sourceBoard).toBe("catho-browser-mcp");
      ...
      expect(job.sourceId).toMatch(/^catho:\d+$/);
      ...
      expect(job.metadata).toEqual({
        browserMcp: { siteId: "catho", query: "desenvolvedor" },
      });
    }

    const ids = result.jobs.map((j) => j.sourceId);
    expect(ids).toEqual(["catho:1234567", "catho:7654321", "catho:9988776"]);
  });
...
describe("CathoAdapter login-wall fixture", () => {
...
describe("CathoAdapter captcha fixture", () => {
...
describe("CathoAdapter pagination", () => {
  it("navega para ?pagina=N em páginas subsequentes", async () => {
    ...
    expect(firstUrl).toContain("catho.com.br/vagas/");
...
describe("CathoAdapter marketHint", () => {
  ...
  it("não define marketHint quando locationText não contém termos brasileiros", async () => {
    const htmlWithForeignLocation = `
      <div class="job-list">
        <article class="job-card" data-job-id="55555">
          <a class="job-card__title-link" href="https://www.catho.com.br/vagas/55555/frontend-dev">
...
describe("CathoAdapter sourceId deduplication", () => {
...
describe("Property 2 — URL round-trip", () => {
  ...
  function buildCathoHtml(jobId: string): string {
    return `
      <div class="job-list">
        <article class="job-card" data-job-id="${jobId}">
          <a class="job-card__title-link" href="https://www.catho.com.br/vagas/${jobId}/test-job-title">
            <h2 class="job-card__title">Test Job</h2>
          </a>
          <p class="job-card__company">Test Company</p>
  ...
  it("URLs emitidas são válidas, absolutas e normalizeJobUrl é idempotente", { timeout: 30_000 }, async () => {
    await fc.assert(
      fc.asyncProperty(jobIdArb, async (jobId) => {
        const html = buildCathoHtml(jobId);
